...
class ServiceController extends Controller
{
    ...
    public function apiServices($id)
    {
        ...
        if(str_contains($url, 'masar')) {
            $url = $url . '/products';
            $client = new Client();

            $response = $client->request('GET', $url, [
                'headers' => [
                    'api-token' => $api->api_key,
                ]
            ])->getBody();
        } else {
        ...
        \Log::debug(['item' => $response[6]]);

        foreach ($response as $value) {
            if(str_contains($url, 'masar')) {
                if(is_array($value->qty_values)) {
                    $values = (array) $value->qty_values;
                    $value->min = $values[0];
                    $value->max = end($values);
                } else if($value->qty_values) {
                    $value->min = $value->qty_values->min;
                    $value->max = $value->qty_values->max;
                } else {
                    $value->min = 1;
                    $value->max = 1;
                }
